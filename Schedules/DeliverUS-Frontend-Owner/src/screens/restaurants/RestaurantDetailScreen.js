...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  const fetchSchedules = async () => {
    try {
      const fetchedSchedule = await getRestaurantSchedules(route.params.id)
      setSchedules(fetchedSchedule)
    } catch (error) {
      showMessage({
        message: `There was an error while retrieving schedules for restaurant (id ${route.params.id}). ${error}`,
        type: 'error',
  ...
  const renderProduct = ({ item }) => {
    const productSchedule = schedules.find(s => s.products.some(p => p.id === item.id))
    const renderSchedule = () => {
      return (
        productSchedule
          ? <View style={styles.scheduleContainer}>
              <MaterialCommunityIcons name='timetable' color={GlobalStyles.brandGreen} size={20}/>
              <TextRegular textStyle={{ color: GlobalStyles.brandGreen }}>{productSchedule.startTime} - {productSchedule.endTime}</TextRegular>
            </View>
          : <View style={styles.scheduleContainer}>
