...
export default function RestaurantSchedulesScreen ({ navigation, route }) {
  ...
  const [schedulesToBeDeleted, setSchedulesToBeDeleted] = useState(null)
  ...
  const renderSchedule = ({ item }) => {
    return (
      ...
      >
        ...
        <TextSemiBold textStyle={styles.productsAssociatedText}>{item.products.length} products associated</TextSemiBold>
                ...
                <Pressable
                    onPress={() => { setSchedulesToBeDeleted(item) }}
                    style={({ pressed }) => [
                      {
                        backgroundColor: pressed
    ...
    />
          <DeleteModal
            ...
            onConfirm={() => remove(schedulesToBeDeleted)}>
              <TextRegular>Estas seguro?</TextRegular>
          </DeleteModal>
    </View>
  )
}
